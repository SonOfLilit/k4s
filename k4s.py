...
class ContainerResource(Resource):
    ...
    def __init__(self, name: str, spec: Dict[str, Any], metadata: Dict[str, Any] = None):
        super().__init__(kind="Container", name=name, spec=spec, metadata=metadata or {})
...
class ReplicaSetResource(Resource):
    ...
    def __init__(self, name: str, spec: Dict[str, Any], metadata: Dict[str, Any] = None):
        super().__init__(kind="ReplicaSet", name=name, spec=spec, metadata=metadata or {})
...
class Container:
    ...
    def __init__(self, name: str, module_name: str, function_name: str, 
                 parameters: Dict[str, Any], api_client: 'KubernetesAPI'):
        self.name = name
    ...
    def _run(self):
        ...
        try:
            ...
            func(input_queue=self.input_queue, api_client=self.api_client, **self.parameters)
...
class ReplicaSetController(Controller):
    ...
    def reconcile(self):
        ...
        for replicaset in self.store.list("ReplicaSet"):
            ...
            if not template:
                print(f"ReplicaSet {replicaset.name}: template container {replicaset.container_name} not found")
                continue
            
            # Find existing replicas
            prefix = f"{replicaset.name}-"
            existing_replicas = [
                r for r in self.store.list("Container")
                if r.name.startswith(prefix) and r.metadata.get("replicaset") == replicaset.name
            ]
...
class ServiceController(Controller):
    ...
    def reconcile(self):
        ...
        for service in self.store.list("Service"):
            ...
            if not matches:
                print(f"Warning: Service {service.name} selector '{service.selector}' matches no containers")
...
class KubernetesAPI:
    ...
    def send_to_container(self, container_name: str, value: Any, 
                         expect_response: bool = False) -> Optional[Future]:
    ...
    def send_to_service(self, service_name: str, value: Any,
                       expect_response: bool = False) -> Optional[Future]:
